...
function breadth_first($needle, $node){
    ...
    while(count($unvisited) > 0){
        $current = $unvisited[0];
        print("Visiting Node " . $current->label . "\n");
...
print_tree($tree);
print("Found id 42 at node " . breadth_first(42, $tree)-> label . "\n");
// EndStudentCode
...
class Question3Test extends PHPUnit\Framework\TestCase {
    public function test() {
        ...
        $this->assertEquals('D', depth_first(2, $tree)->label);
        $this->assertEquals([87, 18, 1, 2], $log_data);
        $log_data = [];
        $this->assertEquals(null, depth_first(16, $tree));
        $this->assertEquals([87, 18, 1, 2, 43, 23, 42], $log_data);
    }
}
